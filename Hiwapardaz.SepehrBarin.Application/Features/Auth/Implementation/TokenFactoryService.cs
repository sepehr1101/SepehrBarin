...
namespace Hiwapardaz.SepehrBarin.Application.Features.Auth.Implementation
{
    public class TokenFactoryService : ITokenFactoryService
    {
        ...
        public TokenFactoryService(
            ...
            IUserRoleQueryService userRolesService,
            ...
            ILogger<TokenFactoryService> logger)
        {
            ...
            _userRoleQueryService = userRolesService;
            _userRoleQueryService.NotNull(nameof(userRolesService));
        ...
        private async Task<(string AccessToken, IEnumerable<Claim> Claims)> CreateAccessTokenAsync(User user)
        {
            var claims = new List<Claim>
            {
                ...
                new Claim(ClaimTypes.Name, user.Mobile, ClaimValueTypes.String, _configuration.Value.Issuer),               
            ...
            // add roles
            var roles = await _userRoleQueryService.Get(user.Id);
            foreach (var role in roles)
            {
                claims.Add(new Claim(ClaimTypes.Role, role.Role.Name, ClaimValueTypes.String, _configuration.Value.Issuer));
            }
